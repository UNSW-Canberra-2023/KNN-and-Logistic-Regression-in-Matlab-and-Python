601,100
602,"}tp_workflow__reject-own__classic_approval"
562,"NULL"
586,
585,
564,
...
#******
#******
#***BEGIN(Prolog): }tp_workflow__reject-own__classic_approval
#******
...
If (cNodeType @= 'N');
	If ((cState @= '0' % cState @= '') & (cgEdit @= 'T'));
		...
		If (DIMIX('}Clients', gNewOwnerId) <> 0);
			...
			#*** Set ownership
			tUserId = DimensionElementPrincipalName('}Clients', gNewOwnerId);
...
ElseIf (cNodeType @= 'C');
	If ((cState @= '1' % cState @= '0' % cState @= '' % cState @= '2') & (cgView @= 'T'));
		...
		#*** Define global variables
		StringGlobalVariable('gNewOwnerId');
		vSubset = 'reject-own_ALL_LEAF_CHILDREN' | pTime;
...
EndIf;
#******
#***END(Prolog): }tp_workflow__reject-own__classic_approval
#******
