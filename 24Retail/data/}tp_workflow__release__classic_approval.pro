601,100
602,"}tp_workflow__release__classic_approval"
562,"NULL"
586,
585,
564,
...
#******
#******
#***BEGIN(Prolog): }tp_workflow__release__classic_approval
#******
...
If (cNodeType @= 'N');
	If ((cState @= '2') & (cgEdit @= 'T'));
		...
		If (((CellGetS('}ClientGroups', TM1User, 'ADMIN') @<> '') % (TM1User @= cCurrentOwnerId)) & (cTakeOwnershipNode @= pNode));

			#*** Clear ownership

			CellPutS('', cStateCube, cStateMember, 'CurrentOwner');
			CellPutS('', cStateCube, cStateMember, 'CurrentOwnerId');
			CellPutS('', cStateCube, cStateMember, 'TakeOwnershipNode');
		...
		Else;
			...
			If (~(cTakeOwnershipNode @= pNode));
				...
				tReturnValue = ExecuteProcess('}tp_error_update_error_cube',
					...
					'pErrorDetails', cNode|',' |cTakeOwnershipNode, 
					'pControl', 'Y');
				ProcessError;
			EndIf;
		EndIf;
...
ElseIf (cNodeType @= 'C');
	If ((cState @= '1' % cState @= '2') & (cgView @= 'T'));
		vVisitCount = vVisitCount + 1;
		vSubset = 'release_ALL_LEAF_CHILDREN' | pTime;
		...
		while (vTempIndex <= vSubsetSize);
			...
			If ((vState @= '2') & (vgEdit @= 'T'));
				If (((CellGetS('}ClientGroups', TM1User, 'ADMIN') @<> '') % (TM1User @= vCurrentOwnerId)) & (vTakeOwnershipNode @= pNode));

					#*** Clear ownership

					CellPutS('', cStateCube, vStateMember, 'CurrentOwner');
					CellPutS('', cStateCube, vStateMember, 'CurrentOwnerId');
					CellPutS('', cStateCube, vStateMember, 'TakeOwnershipNode');

					#*** Set State Member Info
					CellPutS('', vStateCube, vStateMember, 'BeingEdited');
...
EndIf;
#******
#***END(Prolog): }tp_workflow__release__classic_approval
#******
