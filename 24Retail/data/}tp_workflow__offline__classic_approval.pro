601,100
602,"}tp_workflow__offline__classic_approval"
562,"NULL"
586,
585,
564,
...
#******
#******
#***BEGIN(Prolog): }tp_workflow__offline__classic_approval
#******
...
ElseIf (cNodeType @= 'C');
	If ((cState @= '1' % cState @= '3' % cState @= '0' % cState @= '' % cState @= '4' % cState @= '2') & (cgView @= 'T'));
		vVisitCount = vVisitCount + 1;
		vSubset = 'offline_ALL_LEAF_CHILDREN' | pTime;
		...
		while (vTempIndex <= vSubsetSize);
			...
			If ((vState @= '0' % vState @= '' % vState @= '4' % vState @= '2') & (vgView @= 'T'));
				...
				#*** Set State Member Info
				CellPutS('Y', vStateCube, vStateMember, 'Offline');
			EndIf;
...
EndIf;
#******
#***END(Prolog): }tp_workflow__offline__classic_approval
#******
