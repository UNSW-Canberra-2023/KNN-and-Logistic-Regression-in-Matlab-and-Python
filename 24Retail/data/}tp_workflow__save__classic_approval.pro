601,100
602,"}tp_workflow__save__classic_approval"
562,"NULL"
586,
585,
564,
...
#******
#******
#***BEGIN(Prolog): }tp_workflow__save__classic_approval
#******
...
ElseIf (cNodeType @= 'C');
	If ((cState @= '1' % cState @= '2') & (cgView @= 'T'));
		vVisitCount = vVisitCount + 1;
		vSubset = 'save_ALL_LEAF_CHILDREN' | pTime;
		...
		while (vTempIndex <= vSubsetSize);
			...
			If ((vState @= '2') & (vgEdit @= 'T'));
				If ((TM1User @= vCurrentOwnerId) & (vTakeOwnershipNode @= pNode));

					#*** Set info for data saving
					CellPutS('Y', cStateCube, vStateMember, 'Saved');
					CellPutS(cUserDisplayName, cStateCube, vStateMember, 'DataChangeUser');
					CellPutS(pTime, cStateCube, vStateMember, 'DataChangeDate');
				EndIf;
			EndIf;
...
EndIf;
#******
#***END(Prolog): }tp_workflow__save__classic_approval
#******
